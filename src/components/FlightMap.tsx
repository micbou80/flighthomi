...
export default function FlightMap({
  ...
  const hasDestCoords = destinationLat != null && destinationLon != null
  const destPos: [number, number] | null = hasDestCoords ? [destinationLat!, destinationLon!] : null
  ...
  // Fit bounds to track + current position + destination if known
  ...
  return (
    <div className="rounded-lg overflow-hidden border border-gray-800" style={{ height: 280, position: 'relative' }}>
      ...
      >
        ...
        {/* Projected route — dashed line from plane to destination airport */}
        {destPos && (
          <Polyline
        ...
        {destPos && (
          <Marker position={destPos} icon={makeLabel(destinationCode, '#facc15')} />
        )}

        {/* Fallback: destination label at last track point if no airport coords and plane has moved */}
        {!destPos && livePositionDistinct && lastTrack && (
      ...
      {/* Corner badge — only when destination coords unknown (no map label shown) */}
      {!destPos && (
        <div style={{
          position: 'absolute',
          bottom: 8,
